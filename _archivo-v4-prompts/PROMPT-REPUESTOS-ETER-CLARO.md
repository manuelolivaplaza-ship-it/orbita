# SUPER-PROMPT — REPUESTOS · GRUPO "CLARO MINIMALISTA / NEUTRO" — ÉTER CLARO (51 sitios)
...
industrial sobria, acento contenido, el repuesto como protagonista técnico. Es la estética de la
casa de repuestos que vende criterio, stock real y despacho — no humo. Vende que el repuesto
correcto llega hoy, no mañana. Que la compatibilidad se verifica por VIN, no a ojo. Que el
taller no queda parado esperando.

Benchmark REAL verificado del rubro en este mismo segmento (medido por captura renderizada,
68 sitios, 51 en este grupo):
Authievre Motors Las Condes (motors.cl), Multirepuestos Providencia (multirepuestos.cl),
AS Automotriz Providencia (asautomotriz.cl), EuroEspecialista Providencia
(euroespecialista.cl), T5 Parts Providencia (t5parts.cl), Derco Center Tabancura Vitacura
(derco.cl). Internacionalmente, el canónico del repuesto claro premium: RockAuto (rockauto.com)
y Autodoc (autodoc.co.uk) en su versión editorial limpia — aire, tabla técnica, el repuesto
iluminado sobre papel, cero neón.
...
1. Duplica `_plantilla` como base del proyecto (no partas de cero). Está en
   ...
   Copia ARCHIVOS, no node_modules. Nombre de carpeta: `propuestas/repuestos-eter-claro`.
2. En tu copia: package.json → "name": "repuestos-eter-claro". meta.json →
   { "title": "ÉTER REPUESTOS — Casa de Repuestos · Propuesta Órbita", "client": "ÉTER REPUESTOS",
     "brand": "ÉTER REPUESTOS", "sector": "repuestos",
     "description": "Propuesta clara minimalista para casa de repuestos chilena: stock verificable, compatibilidad por VIN y despacho hoy." }.
   index.html → <html lang="es">, <title>ÉTER REPUESTOS — Repuestos con despacho hoy</title>.
...
4. Comandos obligatorios (desde la RAÍZ `C:\Users\manue\OneDrive\Desktop\órbita`):
   - `npm run propuesta -- repuestos-eter-claro`
   - `npm run propuestas:build -- repuestos-eter-claro` (debe compilar limpio antes de terminar).
5. Las imágenes van en TU app: propuestas/repuestos-eter-claro/public/media/.

## MARCA DEMO (textos literales, no cambiar)
Nombre: ÉTER REPUESTOS — Casa de Repuestos
H1: "El repuesto correcto, hoy. Sin vueltas."
Subhead: "Verificamos por VIN, confirmamos stock real y despachamos en el día.
Si no lo tenemos, te decimos al tiro — no te hacemos perder el taller."
CTA principal: "Cotizar por VIN" · secundario: "Ver stock"
Urgencia/Ws: +56 9 8234 5678 (texto visible permanente: "¿Auto detenido? Escríbenos")
Contacto: ventas@eterrepuestos.cl · 10 de Julio 771, Santiago · Despacho RM y regiones
...
ACENTO ÚNICO --rojo-signal #C1272D (<5% de la UI: CTA principal, estado activo, link).
...
y whitespace. Botón sólido --tinta sobre --papel, hover a --rojo-signal con transición 180ms.

## PASO 0 — MEDIA (public/media/, máx 4 imágenes, si faltan tipografía lleva el layout)
mostrador.jpg 16:9 mostrador técnico luminoso y VACÍO, estanterías con cajas alineadas, luz norte, sin personas ·
detalle.jpg 1:1 macro de disco de freno nuevo sobre papel kraft claro, luz rasante suave ·
bodega.jpg 16:9 pasillo de bodega claro ordenado, cajas rotuladas por código, simetría industrial ·
kit.jpg 4:5 bodegón de filtros y pastillas alineados sobre superficie hueso, sombra suave.
PROHIBIDO: mecánico sonriendo con pulgar arriba, manos con llave inglesa, familia con auto,
renders 3D futuristas, repuestos flotando con brillo neón. Si una imagen no convence, se descarta
y el layout tipográfico resuelve.

## SECCIONES — ARCO DE CONVERSIÓN (ids fijos, anclas del nav SIEMPRE funcionan)
#inicio    Hero editorial 12cols: kicker uppercase tracking .18em ("CASA DE REPUESTOS · 10 DE JULIO · DESDE 2008"),
           H1 gigante leading 0.92 tracking -0.03em, subhead 2 líneas gris, CTA sólido tinta "Cotizar por VIN"
           + link subrayado animado a #precios "Ver precios referencia". Mostrador.jpg derecha 7/12 con caption
...
#compatibilidad  "¿Le hace a mi auto? Lo verificamos por VIN." Buscador editorial por VIN/patente + marca/modelo/año
           ...
           --rojo-signal). 3 columnas filete superior: 01 Envías VIN → 02 Confirmamos código exacto → 03 Retiras o
...
#precios   "Precios de referencia, sin sorpresas": tabla editorial 6 filas (NO cards SaaS) con precio DESDE en CLP
           ...
           filete --rojo-signal 2px a la izquierda (solo una).
#stock     Grilla 3×2 de categorías con foto detalle/bodega como pieza técnica: Frenos · Suspensión/Dirección ·
...
#reserva   Headline "¿Auto detenido? Hablemos ahora." + teléfono tabular gigante + CTA Ws + horarios + dirección con
           micro-mapa línea (no iframe pesado). Footer sobrio legal CL: razón social ÉTER REPUESTOS SpA, RUT, SII, año,
           "10 de Julio 771, Santiago — Estacionamiento clientes".

## CONVERSIÓN SIEMPRE PRESENTE
Nav con botón "Cotizar por VIN" fijo + teléfono visible en desktop; nav hide-down/show-up + compacta 24px al scroll.
Sticky CTA móvil discreto tras el hero ("Cotizar por VIN" con icono VIN minimal línea 16px). Subrayados animados 200ms.
Barra progreso scroll 2px var(--rojo-signal).
...
- Acento --rojo-signal en MENOS del 5% de la UI. Radios 0. Botón sólido tinta sobre claro.
- Todo texto y alt en español de Chile, alt="" descriptivos, contraste AA, focus-visible ring rojo-signal,
  ::selection con rojo-signal/texto papel, hero impecable a 360px, padding vertical ≥112px desktop / ≥72px móvil,
...
repuestos-eter-claro, revisa en navegador y corrige (360px, 768px, 1280px). 5) npm run propuestas:build --
repuestos-eter-claro hasta cero errores. No agregues dependencias; no toques package.json ni vite.config.ts
...
(grupo de 51 sitios con esta estética clara minimalista). Si queda "bien pero genérica", itera hasta que
