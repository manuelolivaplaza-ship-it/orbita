...
/* ------------------------------------------------------------------ */
/* Spawn / appear: burbuja que revienta + cabeza mareada                */
/*                                                                     */
/* Timeline total (~1600ms):                                           */
/*  1. Pop burbuja: escala 0 -> overshoot (~1.18) -> wobble -> 1        */
/*  2. Mareo: la cara sacude la cabeza con oscilación amortiguada       */
/*                                                                     */
/* Todo es función pura de `progress` (0..1) para poder manejarlo por   */
/* rAF, CSS o cualquier motor en otro proyecto.                        */
/* ------------------------------------------------------------------ */

export const SPAWN_DURATION = 1600;

export function spawnProgress(elapsedMs: number, durationMs = SPAWN_DURATION): number {
  if (durationMs <= 0) return 1;
  return clamp01(elapsedMs / durationMs);
}

/** easeOutBack con overshoot fuerte = sensación de burbuja. */
...
/**
 * Escala + opacidad del pop.
 * 0 -> invisible, 0.32 -> pico burbuja (~1.18), luego wobble hasta 1.
 */
export function spawnScale(progress: number): { scale: number; opacity: number } {
  const p = clamp01(progress);
  if (p === 0) return { scale: 0, opacity: 0 };
  if (p >= 1) return { scale: 1, opacity: 1 };
  if (p < 0.32) {
    const t = p / 0.32;
    return { scale: Math.max(0, easeOutBack(t) * 1.18), opacity: Math.min(1, t * 1.4) };
  }
  if (p < 0.55) {
    const t = (p - 0.32) / 0.23;
    const k = t * t * (3 - 2 * t);
    return { scale: 1.18 - 0.24 * k, opacity: 1 };
  }
  if (p < 0.76) {
    const t = (p - 0.55) / 0.21;
    const k = Math.sin(t * Math.PI);
    return { scale: 0.94 + 0.1 * k + 0.06 * t, opacity: 1 };
  }
  const t = (p - 0.76) / 0.24;
  const k = 1 - (1 - t) ** 2;
  return { scale: 1.04 - 0.04 * k, opacity: 1 };
}

/**
 * Sacudida de cabeza de mareado: oscilación amortiguada que arranca
 * cuando termina el pop. Positivo = inclina a un lado, negativo al otro.
 */
export function spawnTilt(progress: number, maxDeg = 16): number {
  const p = clamp01(progress);
  if (p <= 0.28 || p >= 1) return 0;
  const local = (p - 0.28) / 0.72;
  const decay = (1 - local) ** 1.25;
  return Math.sin(local * Math.PI * 6) * maxDeg * decay;
}

/** Acompaña al mareo: se ensancha/estira como gelatina. */
export function spawnSquish(progress: number): { sx: number; sy: number } {
  const p = clamp01(progress);
  if (p <= 0.28 || p >= 1) return { sx: 1, sy: 1 };
  const local = (p - 0.28) / 0.72;
  const decay = 1 - local;
  const wave = Math.sin(local * Math.PI * 3) * 0.09 * decay;
  return { sx: 1 + wave, sy: 1 - wave };
}

/** Los ojos también tambalean un poco: vende el "quedó mareado". */
export function spawnEyeOffset(progress: number): Point {
  const p = clamp01(progress);
  if (p <= 0.3 || p >= 1) return { x: 0, y: 0 };
  const local = (p - 0.3) / 0.7;
  const decay = 1 - local;
  return {
    x: Math.sin(local * Math.PI * 6) * 2.4 * decay,
    y: Math.cos(local * Math.PI * 4) * 1.2 * decay,
  };
}

/** Aro de burbuja que se expande y se desvanece en la primera mitad. */
export function spawnRing(progress: number): { scale: number; opacity: number } {
  const p = clamp01(progress);
  if (p === 0) return { scale: 0.55, opacity: 0 };
  if (p >= 0.5) return { scale: 1.5, opacity: 0 };
  const t = p / 0.5;
  return { scale: 0.55 + 0.95 * t, opacity: 0.55 * (1 - t * t) };
}

/** true mientras la animación de aparición sigue corriendo. */
export function spawnIsActive(progress: number): boolean {
