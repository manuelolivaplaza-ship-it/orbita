import { mkdir, readFile, writeFile } from "node:fs/promises";
import path from "node:path";
import { NextResponse } from "next/server";

const file = path.join(process.cwd(), "data", "horas.json");

type Hora = {
  name: string;
  phone: string;
  email: string;
  pet: string;
  species: string;
  reason: string;
  date: string;
  notes: string;
  createdAt: string;
};

function isString(v: unknown): v is string {
  return typeof v === "string";
}

export async function POST(request: Request) {
  let body: unknown;
  try {
    body = await request.json();
  } catch {
    return NextResponse.json({ error: "JSON inválido" }, { status: 400 });
  }

  if (!body || typeof body !== "object") {
    return NextResponse.json({ error: "Solicitud incompleta" }, { status: 400 });
  }

  const payload = body as Record<string, unknown>;
  const name = payload.name;
  const phone = payload.phone;

  if (!isString(name) || !isString(phone) || !name.trim() || !phone.trim()) {
    return NextResponse.json({ error: "Faltan datos" }, { status: 400 });
  }

  const hora: Hora = {
    name: name.trim(),
    phone: phone.trim(),
    email: isString(payload.email) ? payload.email.trim() : "",
    pet: isString(payload.pet) ? payload.pet.trim() : "",
    species: isString(payload.species) ? payload.species : "",
    reason: isString(payload.reason) ? payload.reason : "",
    date: isString(payload.date) ? payload.date : "",
    notes: isString(payload.notes) ? payload.notes.trim() : "",
    createdAt: new Date().toISOString(),
  };

  await mkdir(path.dirname(file), { recursive: true });
  let existing: Hora[] = [];
  try {
    existing = JSON.parse(await readFile(file, "utf8")) as Hora[];
    if (!Array.isArray(existing)) existing = [];
  } catch {
    existing = [];
  }
  existing.push(hora);
  await writeFile(file, JSON.stringify(existing, null, 2), "utf8");

  return NextResponse.json({ ok: true });
}
