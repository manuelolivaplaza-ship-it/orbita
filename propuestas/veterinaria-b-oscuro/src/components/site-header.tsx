    ...
    >
      <div className="shell flex h-[4.5rem] items-center justify-between gap-4">
          ...
          {nav.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "text-[0.78rem] tracking-[0.05em] text-muted transition-colors duration-200 hover:text-paper",
                  active && "text-paper"
                )}
              >
                {item.label}
        ...
        <div className="hidden items-center gap-5 lg:flex">
          <a
            href={site.phoneHref}
            className="tabular text-[0.86rem] text-paper transition-colors hover:text-lantern"
          >
            {site.phone}
          </a>
          <Link href="/hora" className="btn btn-primary !min-h-10 !px-5">
            Pedir hora
          </Link>
        </div>
        <button
      ...
      {open ? (
        <div className="fixed inset-0 top-[4.5rem] z-40 bg-void lg:hidden">
          <nav className="shell flex flex-col pt-8" aria-label="Móvil">
            ...
            <Link href="/hora" className="btn btn-primary mt-8 w-full">
              Pedir hora
            </Link>
            <Link href="/urgencias" className="btn btn-ghost mt-3 w-full">
              Es urgente
            </Link>
            <a href={site.phoneHref} className="mt-5 text-center tabular text-paper-dim">
              {site.phone}
            </a>
          </nav>
        </div>
