...
export function ConsultForm({ className }: { className?: string }) {
  ...
  if (status === "sent") {
    return (
      <div className={cn("border border-line bg-luz px-7 py-10", className)} role="status">
        ...
        <h3 className="font-display mt-4 text-3xl font-bold leading-tight tracking-tight">
          Le respondemos dentro de las 24 horas hábiles.
        </h3>
        <p className="mt-4 max-w-md text-[16px] leading-relaxed text-muted">
          Si el asunto no es nuestro, se lo diremos en esa llamada — y a quién
          conviene escribir.
        </p>
      </div>
    );
  ...
  const fieldClass =
    "w-full border border-line bg-luz px-4 py-3 text-[16px] text-ink placeholder:text-muted/70 transition-colors focus:border-barro";
  ...
  return (
    <form onSubmit={onSubmit} className={cn("space-y-5", className)} noValidate>
      ...
      <Field label="Sala" error={errors.area}>
        <select
          className={fieldClass}
          value={values.area}
          ...
          name="area"
        >
          <option value="">La que corresponda</option>
          {practices.map((item) => (
      ...
      {status === "error" ? (
        <p className="text-[14px] text-barro">
          No se pudo enviar. Escríbanos a hola@atrio.cl o por WhatsApp.
        </p>
      ) : null}
      <button
        type="submit"
        disabled={status === "sending"}
        className="font-display inline-flex h-12 items-center bg-barro px-7 text-[0.92rem] font-semibold text-luz transition-colors hover:bg-barro-deep disabled:opacity-60"
      >
...
function Field({
  ...
  return (
    <label className="block">
      <span className="font-display mb-1.5 block text-[12px] font-semibold tracking-wide text-muted">
        {label}
      </span>
      {children}
      {error ? <span className="mt-1 block text-[13px] text-barro">{error}</span> : null}
    </label>
  );
}
