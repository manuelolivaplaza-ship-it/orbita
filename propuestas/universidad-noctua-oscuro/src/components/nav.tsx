...
import { OpenBadge } from "@/components/night-bar";
...
export function Nav() {
  ...
  return (
    <header
      className={cn(
        "fixed inset-x-0 top-0 z-40 text-paper transition-colors duration-500",
    ...
    >
      <div className="mx-auto flex h-[4.5rem] max-w-[1440px] items-center justify-between px-6 md:px-10 lg:px-16">
        ...
        <nav className="hidden items-center gap-9 xl:gap-10 lg:flex" aria-label="Principal">
          {navLinks.map((link) => {
            const active =
      ...
      <div
        id="menu-movil"
        className={cn(
          "fixed inset-0 z-30 bg-void text-paper transition-opacity duration-500 lg:hidden",
      ...
      >
        ...
        <nav className="flex flex-col gap-8 px-6 pt-10" aria-label="Móvil">
          ...
          <Link href="/admision" className="btn btn-amber mt-6 w-fit">
            Admisión {site.admissionYear}
          </Link>
        </nav>
      </div>
