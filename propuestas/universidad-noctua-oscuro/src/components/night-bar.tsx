...
import { isInstituteOpen, moonPhaseName, santiagoParts } from "@/lib/night";
      ...
      <div className="mx-auto flex max-w-[1440px] flex-wrap items-center justify-between gap-3 px-6 py-3 font-mono text-[0.58rem] uppercase tracking-[0.24em] text-muted md:px-10 lg:px-16">
        ...
        <p className="flex items-center gap-3">
          ...
          <span className={state?.open ? "text-amber" : undefined}>
            {state?.open ? "Recinto abierto" : "Recinto en silencio"}
          </span>
...
export function OpenBadge() {
  ...
  useEffect(() => {
    const tick = () => setOpen(isInstituteOpen());
    tick();
  ...
  return (
    <span
      ...
      title={open ? "El recinto está abierto ahora" : "El recinto abre a las 17:30"}
    >
      <span
        className={`h-1.5 w-1.5 rounded-full ${open ? "bg-amber" : "bg-line"}`}
      />
      {open ? "Abierto" : "En silencio"}
    </span>
  );
}
