...
export function BeforeAfter({
  ...
  return (
    <div
      className={cn(
        "relative aspect-[3/4] overflow-hidden rounded-[1.4rem] bg-mist",
        className
      )}
    >
      <Image
      ...
      <div
        className="pointer-events-none absolute inset-y-0 z-10 w-px bg-cian"
        style={{ left: `${pos}%` }}
      />
      <div
        className="pointer-events-none absolute top-1/2 z-10 size-10 -translate-x-1/2 -translate-y-1/2 rounded-full border border-cian/70 bg-card/90 shadow-sm backdrop-blur-sm"
        style={{ left: `${pos}%` }}
      >
