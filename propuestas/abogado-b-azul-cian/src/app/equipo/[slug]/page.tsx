...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: person.name,
    description: `${person.name}, ${person.role} de CAUCE. ${person.practice}.`,
  };
}
      ...
      <section className="pt-28 pb-16 lg:pt-36 lg:pb-24">
        <div className="shell grid items-start gap-10 lg:grid-cols-12 lg:gap-12">
          ...
          <Reveal delay={0.1} className="lg:col-span-6 lg:col-start-7 lg:pt-8">
            ...
            <div className="waterline waterline-draw mt-7 w-32" />
            ...
            {practice ? (
              <Link
                ...
                className="mt-6 inline-flex text-[0.92rem] font-semibold text-cyan-deep"
              >
                Afluente de {practice.title} →
              </Link>
            ) : null}
          </Reveal>
        </div>
      ...
      <section className="border-t border-line py-16">
        <div className="shell flex flex-wrap items-center justify-between gap-4">
          <p className="font-display text-2xl font-semibold tracking-tight">
            Pedir un sondaje con {person.name.split(" ")[0]}
          </p>
          <div className="flex gap-3">
            <a
              href={site.whatsapp}
              className="inline-flex h-12 items-center border border-ink px-5 text-[0.9rem] font-semibold"
            >
              WhatsApp
            </a>
            <Link
              href="/contacto"
              className="inline-flex h-12 items-center bg-navy px-5 text-[0.9rem] font-semibold text-paper"
            >
              Escribir
            </Link>
          </div>
          ...
          <div className="mt-8 grid grid-cols-2 gap-4 md:grid-cols-4">
            {others.map((item) => (
              <Link key={item.slug} href={`/equipo/${item.slug}`} className="group block">
                ...
                <p className="font-display mt-3 font-semibold group-hover:text-cyan-deep">
                  {item.name}
                </p>
