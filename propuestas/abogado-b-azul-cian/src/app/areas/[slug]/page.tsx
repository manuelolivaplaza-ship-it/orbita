  ...
  return (
    <>
      <section className="pt-32 pb-12 lg:pt-40">
        <div className="shell">
          <Reveal>
            <p className="kicker">Prof. {practice.depth}</p>
            <h1 className="font-display mt-4 max-w-[14ch] text-[clamp(2.8rem,7vw,5.6rem)] font-semibold leading-[0.92] tracking-tight">
              {practice.title}
            </h1>
            <div className="waterline waterline-draw mt-7 w-40" />
          ...
          <Reveal delay={0.1} className="lg:col-span-5 lg:col-start-8">
            ...
            <ul className="mt-6 space-y-4">
              {practice.when.map((item) => (
                <li
                  key={item}
                  className="border-l-2 border-cyan pl-4 text-[16px] leading-relaxed"
                >
                  {item}
                </li>
              ))}
            ...
            {lawyer ? (
              <Link href={`/equipo/${lawyer.slug}`} className="group mt-12 flex items-center gap-4">
                ...
                <span>
                  <span className="font-display block text-lg font-semibold group-hover:text-cyan-deep">
                    {lawyer.name}
                  </span>
                  <span className="text-[13px] text-muted">
                    {lawyer.role} · lleva este afluente
                  </span>
                </span>
              </Link>
            ) : null}
            <Link
              href="/contacto"
              className="mt-10 inline-flex h-12 items-center bg-navy px-6 text-[0.9rem] font-semibold text-paper hover:bg-ink"
            >
              Pedir sondaje
            </Link>
          </Reveal>
      ...
      <section className="border-t border-line py-16 lg:py-20">
        <div className="shell">
          <p className="kicker">Otros afluentes</p>
          <ul className="mt-8 divide-y divide-line border-y border-line">
            {others.map((item) => (
              <li key={item.slug}>
                ...
                >
                  <span className="font-display text-2xl font-semibold tracking-tight group-hover:text-cyan-deep">
                    {item.title}
                  </span>
                  <span className="text-cyan-deep transition-transform duration-500 group-hover:translate-x-1">
                    →
                  </span>
                </Link>
              </li>
