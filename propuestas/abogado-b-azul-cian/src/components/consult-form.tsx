...
export function ConsultForm({ className }: { className?: string }) {
  ...
  if (status === "sent") {
    return (
      <div className={cn("border border-line bg-paper px-7 py-10", className)} role="status">
  ...
  const fieldClass =
    "w-full border border-line bg-paper px-4 py-3 text-[16px] text-ink placeholder:text-muted/70 transition-colors focus:border-cyan";
  ...
  return (
    <form onSubmit={onSubmit} className={cn("space-y-5", className)} noValidate>
      ...
      <Field label="Afluente" error={errors.area}>
        <select
          className={fieldClass}
          value={values.area}
      ...
      {status === "error" ? (
        <p className="text-[14px] text-blue">
          No se pudo enviar. Escríbanos a hola@cauce.cl o por WhatsApp.
        </p>
      ) : null}
      <button
        type="submit"
        disabled={status === "sending"}
        className="inline-flex h-12 items-center bg-navy px-7 text-[0.92rem] font-semibold text-paper transition-colors hover:bg-ink disabled:opacity-60"
      >
        {status === "sending" ? "Enviando…" : "Pedir un sondaje"}
      </button>
      <p className="text-[13px] leading-relaxed text-muted">
        El sondaje cuesta $48.000 y se descuenta si tomamos el asunto.
...
function Field({
  ...
  return (
    <label className="block">
      ...
      {children}
      {error ? <span className="mt-1 block text-[13px] text-blue">{error}</span> : null}
    </label>
  );
}
