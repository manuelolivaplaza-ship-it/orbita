...
import { bodegaAbierta, formatSantiagoClock, proximoCierre } from "@/lib/turno";
...
export function Lamp({ className }: { className?: string }) {
  ...
  useEffect(() => {
    const tick = () => {
      const now = new Date();
      setOpen(bodegaAbierta(now));
      setCierre(proximoCierre(now));
      setClock(formatSantiagoClock(now));
    };
    tick();
  ...
  if (open === null) {
    return (
      <p
        className={cn(
          "flex items-center gap-2 font-mono text-[0.62rem] uppercase tracking-[0.2em] text-mute",
          className,
        )}
      >
        <span className="lamp" />
