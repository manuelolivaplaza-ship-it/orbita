  ...
  return (
    ...
    >
      ...
      <div className="wrap flex h-[4.25rem] items-center justify-between md:h-[4.75rem]">
        <Link href="/" aria-label="Alba, inicio" className="relative z-50">
          <Logo />
        </Link>
        ...
        <div className="hidden md:block">
          <Link
            href="/contacto"
            className="inline-flex h-10 items-center rounded-full bg-ink px-4 text-[0.82rem] tracking-[0.04em] text-foam transition-colors hover:bg-copper"
          >
            Conversemos
          </Link>
        </div>
          ...
          <nav className="flex flex-col gap-2" aria-label="Móvil">
            {nav.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="font-display text-5xl tracking-[-0.04em] text-ink"
              >
                {item.label}
              </Link>
            ))}
            <Link
              href="/contacto"
              className="font-display text-5xl tracking-[-0.04em] text-copper"
            >
              Conversemos
            </Link>
          </nav>
          <p className="eyebrow">Santiago · Lastarria</p>
        </div>
      </div>
    </header>
  );
