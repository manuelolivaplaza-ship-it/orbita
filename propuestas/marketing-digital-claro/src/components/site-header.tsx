    ...
    >
      <div className="shell flex h-[4.25rem] items-center justify-between lg:h-20">
          ...
          {nav.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "text-[0.84rem] font-semibold tracking-wide transition-colors",
        ...
        <div className="flex items-center gap-3">
          <a
            href={site.phoneHref}
            className="hidden text-[0.84rem] font-semibold tracking-wide text-ink hover:text-cobre sm:inline"
          >
            {site.phone}
          </a>
          <Link
            href="/contacto"
            className="hidden h-10 items-center bg-sol px-5 text-[0.84rem] font-semibold text-ink transition-colors hover:bg-sol-deep sm:inline-flex"
          >
            Pedir un brief
          </Link>
          <button
      ...
      {open ? (
        <div className="border-t border-line bg-luz lg:hidden">
          <nav className="shell flex flex-col gap-1 py-8" aria-label="Móvil">
            ...
            <Link
              href="/contacto"
              className="mt-4 inline-flex h-12 items-center justify-center bg-sol text-[0.95rem] font-semibold text-ink"
            >
              Pedir un brief
            </Link>
            <a href={site.whatsapp} className="py-3 text-mar">
              WhatsApp
            </a>
          </nav>
        </div>
