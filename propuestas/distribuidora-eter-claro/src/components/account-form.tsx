...
const STORAGE_KEY = "eter-cuenta-solicitud";
...
export function AccountForm() {
  ...
  const whatsapp = useMemo(() => {
    ...
    const text = [
      "Hola ETER, quiero abrir cuenta comercial.",
    ...
    return `https://wa.me/56978621140?text=${encodeURIComponent(text)}`;
  }, [payload]);
  ...
  if (status === "sent") {
    return (
      <div className="border border-line bg-paper-2/70 px-6 py-14 md:px-10">
        <p className="font-mono text-[0.62rem] uppercase tracking-[0.24em] text-frost">
          Solicitud en el CD
        </p>
        ...
        <p className="mt-6 max-w-md text-sm leading-relaxed text-ink-soft">
          Te escribimos en horario de CD, normalmente el mismo día hábil. Si
          quieres adelantar ficha o volumen, ábrelo por WhatsApp.
        </p>
    ...
    <form onSubmit={onSubmit} className="grid gap-8">
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <label className="block">
          ...
          />
          {rutError ? (
            <span className="mt-2 block text-xs text-frost">{rutError}</span>
          ) : null}
        </label>
      </div>
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <label className="block">
          ...
          <select name="volumen" className="input-line mt-2" defaultValue="180-400">
            <option value="bajo-minimo">Menos del pedido mínimo</option>
            <option value="180-400">$180.000 a $400.000</option>
      ...
      <label className="block">
        ...
        <textarea
          name="nota"
          className="input-line mt-2"
          placeholder="Estados, frecuencias, restricciones de horario en el local."
        />
      </label>
