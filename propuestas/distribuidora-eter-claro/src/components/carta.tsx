...
export function Carta({
  ...
  return (
    <div role="table" aria-label="Carta de productos">
      <div
        className="hidden border-b border-line pb-3 font-mono text-[0.58rem] uppercase tracking-[0.2em] text-muted md:grid md:grid-cols-[6.2rem_1.4fr_1fr_7.5rem_7.2rem] md:gap-4"
        role="row"
      >
        <span>SKU</span>
        <span>Producto</span>
      ...
      {items.map((item) => {
        const estado = estados.find((entry) => entry.id === item.estado);
        return (
          <div key={item.sku} className="carta-row" role="row">
            <span role="cell" className="font-mono text-[0.68rem] tracking-wide text-muted">
              {item.sku}
            </span>
            <span role="cell">
              ...
              <span className="mt-1 block text-sm text-muted">
                {item.producer}
                {showEstado && estado ? ` · ${estado.name}` : ""}
              </span>
            </span>
            <span role="cell" className="hidden text-sm text-ink-soft md:block">
              {item.origin}
            </span>
            <span role="cell" className="hidden text-sm text-ink-soft md:block">
              {item.format}
            </span>
            <span role="cell" className="text-right font-mono text-[0.86rem] tabular-nums">
