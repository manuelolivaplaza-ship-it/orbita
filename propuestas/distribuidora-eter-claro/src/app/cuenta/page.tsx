...
export const metadata: Metadata = {
  title: "Abrir cuenta",
  description:
    "Solicitud de cuenta comercial ETER. RUT, giro, comuna y volumen. Pedido mínimo y plazos claros.",
};

const requisitos = [
  "RUT de empresa y giro vigente en SII.",
  "Dirección de cocina o bodega con horario de recepción.",
  "Encargado de compras con teléfono que conteste de mañana.",
          ...
          <ul className="mt-10 space-y-4 text-sm leading-relaxed text-ink-soft">
            {requisitos.map((item) => (
              <li key={item} className="grid grid-cols-[0.7rem_1fr] gap-3">
                <span className="mt-2 block h-px bg-frost" />
                <span>{item}</span>
              </li>
            ))}
          </ul>
