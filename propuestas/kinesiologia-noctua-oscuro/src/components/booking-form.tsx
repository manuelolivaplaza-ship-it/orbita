...
import { previsiones, services, slots } from "@/data/content";
...
export function BookingForm({ preset }: { preset?: string }) {
  ...
  const fromQuery = searchParams.get("enfoque");
  const initial =
    preset ??
    (fromQuery && services.some((item) => item.slug === fromQuery)
      ? fromQuery
      : "lectura");
  const [sent, setSent] = useState(false);
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [rut, setRut] = useState("");
  const [enfoque, setEnfoque] = useState(initial);
  ...
  function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const enfoqueValue = String(data.get("enfoque") ?? "").trim();
    ...
    if (!enfoqueValue) next.enfoque = "Elija un oficio, o la lectura.";
    ...
    return (
      <div className="border border-line bg-surface px-8 py-16">
        ...
        <p className="mt-6 max-w-md text-sm leading-relaxed text-paper-dim">
          ...
          cupo. Responden kinesiólogos, no un call center. Si prefieres
          adelantar, escríbenos ahora.
        </p>
    ...
    <form onSubmit={onSubmit} className="grid gap-8" noValidate>
      ...
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <label className="block">
          ...
          <input
            ...
            autoComplete="email"
            className="input-line mt-2"
            placeholder="catalina@correo.cl"
          />
          {errors.correo ? (
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <label className="block">
          ...
          {errors.enfoque ? (
            <span className="mt-2 block text-xs text-amber">{errors.enfoque}</span>
          ) : null}
        </label>
        <label className="block">
          <span className="font-mono text-[0.62rem] uppercase tracking-[0.24em] text-muted">
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <label className="block">
          ...
          <select name="cupo" className="input-line mt-2" defaultValue="21:00">
            {slots.map((slot) => (
      ...
      <label className="block">
        ...
        <textarea
          name="nota"
          className="input-line mt-2"
          placeholder="El hombro, la lumbar, el post operatorio. En tu idioma."
        />
      </label>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <p className="max-w-sm text-xs leading-relaxed text-muted">
          ...
          confirma en la lectura, no antes.
        </p>
