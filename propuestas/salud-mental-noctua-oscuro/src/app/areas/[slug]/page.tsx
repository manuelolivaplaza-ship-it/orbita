      ...
      <section className="shell grid gap-12 pb-16 pt-32 md:grid-cols-12 md:pb-20 md:pt-40">
        <div className="md:col-span-6">
          ...
          <div className="mt-10 flex flex-wrap gap-4">
            <Link href={`/primera?area=${area.slug}`} className="btn btn-amber">
              Pedir primera hora
              <Arrow />
            </Link>
      ...
      <section className="border-t border-line py-24 lg:py-32">
        <div className="shell grid gap-12 lg:grid-cols-12">
          ...
          <Reveal className="lg:col-span-5 lg:col-start-8" delay={100}>
            ...
            <ul className="mt-6 space-y-4 border-t border-line">
              {area.includes.map((item) => (
                <li key={item} className="border-b border-line py-4 text-paper-dim">
                  {item}
                </li>
              ))}
            </ul>
            ...
            <div className="mt-8 grid gap-8 md:grid-cols-3">
              {people.map((person) => (
                <Link key={person.slug} href={`/equipo/${person.slug}`} className="group">
                  ...
                  <h3 className="mt-4 font-display text-3xl font-semibold tracking-tight group-hover:text-amber">
                    {person.name}
                  </h3>
          ...
          <div className="mt-8 grid gap-8 md:grid-cols-3">
            {others.slice(0, 3).map((item) => (
              <Link key={item.slug} href={`/areas/${item.slug}`} className="group">
                ...
                <h3 className="mt-3 font-display text-3xl font-semibold tracking-tight group-hover:text-amber">
                  {item.title}
                </h3>
                <p className="mt-3 text-sm leading-relaxed text-muted">{item.forWhom}</p>
              </Link>
            ))}
          </div>
        </div>
